...
public partial class stock_instrumentosSeguimiento : System.Web.UI.Page
{
    ...
    {
        bool result = false;
        string path = @"\\10.0.0.4\Usuarios\Liliana.villamil\SGI MULTISITIO\03 Listas de control\FA\FA-020 Lista de control de equipos\EQUIPOS SGI\" + numeroInst + "\\" + numeroInst + ".pdf";
    ...
    public static object[][] GetListaSeguimiento(int pagina, int numeroI, int tipo, string descripcion, string marca)
    {
        ...
        foreach (object[] fila in filas)
        {
            result.Add(
                new object[] { 
                    Encriptacion.GetParametroEncriptado(fila[0].ToString()), 
                    fila[11], 
                    fila[1], 
                    fila[2], 
                    fila[3], 
                    fila[4], 
                    GHerramientas.GetFrecuenciaCalibracion(Convert.ToInt32(fila[5])), 
                    fila[6], 
                    fila[7], 
                    fila[8],
                    GHerramientas.GetTipoCalibracion(Convert.ToInt32(fila[9])),
                    Encriptacion.GetParametroEncriptado("path=" + fila[11] + "\\" + fila[11] + ".jpg&idPath=" + (int)PathImage.ListadoInstrumentos),
